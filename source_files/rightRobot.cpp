 ...
 *
 *
 * RIGHT ROBOT CLASS METHODS
 *
 *
 ************************************************************************************/

// RightRobot Constructor
// RightRobot must be given a (x, y, z) spawn coordinates
RightRobot::RightRobot(float x)
{
	// position.y never changes
	// RightRobot always stays above the ground when initialized
	position.x = x;
...
// All RightRobot Animations Here
void RightRobot::animate(int delay)
{
...
// RightRobot Walking Animation
void RightRobot::walkAnimation()
{
...
// RightRobot should never go past the player
// Respawn if health = 0
void RightRobot::respawn()
{
	int* delaysPtr = generateAnimationDelay(50);
	float deltaTime = currTime - oldTime;

	// RightRobot has passed the Player
	// Delay the respawn
...
// Bobs RightRobot head up and down
void RightRobot::headAnimation()
{
...
void RightRobot::shuffleAnimation()
{
