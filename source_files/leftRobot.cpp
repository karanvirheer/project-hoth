 ...
 *
 *
 * LEFT ROBOT CLASS METHODS
 *
 *
 ************************************************************************************/

// LeftRobot Constructor
// LeftRobot must be given a (x, y, z) spawn coordinates
LeftRobot::LeftRobot(float x)
{
	// position.y never changes
	// LeftRobot always stays above the ground when initialized
	position.x = x;
...
// All LeftRobot Animations Here
void LeftRobot::animate(int delay)
{
...
// LeftRobot Walking Animation
void LeftRobot::walkAnimation()
{
...
// LeftRobot should never go past the player
// Respawn if health = 0
void LeftRobot::respawn()
{
	int* delaysPtr = generateAnimationDelay(50);
	float deltaTime = currTime - oldTime;

	// LeftRobot has passed the Player
	// Delay the respawn
...
// Bobs LeftRobot head up and down
void LeftRobot::headAnimation()
{
...
void LeftRobot::shuffleAnimation()
{
